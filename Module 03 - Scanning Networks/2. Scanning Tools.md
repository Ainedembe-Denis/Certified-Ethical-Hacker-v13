...
> **About these notes:** A practical, exam-ready reference covering the primary network scanning tools used in CEH — their capabilities, key commands, response logic, and exam traps.

---

## 📑 Table of Contents

1. [Nmap — The Core Scanner](#1-nmap--the-core-scanner)
...
2. [Hping3 — Packet Crafting Specialist](#2-hping3--packet-crafting-specialist)
...
3. [Metasploit — Framework with Scanning Modules](#3-metasploit--framework-with-scanning-modules)
...
4. [NetScanTools Pro — Multi-Function Suite](#4-netscantools-pro--multi-function-suite)
...
7. [CEH Exam Tips — Scanning Tools](#7-ceh-exam-tips--scanning-tools)

---

## 1. Nmap — The Core Scanner
...
> 🔥 **Nmap is the single most important scanning tool in the CEH exam.** It appears in nearly every scanning scenario.
...
Nmap operates on a simple **Probe → Observe → Infer** model:

```
Step 1:  Nmap sends specially crafted packets to the target
              ↓
Step 2:  Target responds — or stays silent
              ↓
...
Step 4:  Nmap infers:
           ...
           ├── OS type and version
           ├── Running service and version
           └── Firewall / filter behavior
```

> 📌 This entire inference process relies on **TCP/IP stack fingerprinting** — each operating system implements TCP slightly differently, and Nmap exploits these differences to identify targets.

---

### 🔍 What Attackers Extract Using Nmap
...
> 💡 Together, this output builds the attacker's **target profile** — the intelligence needed to move from scanning into exploitation.
...
> 📌 *Exam note:* Often referred to as a "basic TCP scan."
...
> 📌 Matches the **host discovery stage** of the module. Use this before a full port scan to avoid wasting time on dead IPs.
...
| Flag | Meaning |
|:----:|---------|
| `-sn` | Host discovery only — skip port scan |
| `-PE` | Use ICMP Echo Request (standard ping) |

> ⚠️ ICMP is frequently blocked by firewalls. Use alternative methods below when it fails.

---

#### 🔹 ARP Ping Scan — Best for LAN
```bash
...
> 📌 **CEH Exam Fact:** The default Nmap UDP ping probe port is **40125**. This specific value appears in exam questions.
...
> ⚠️ **Warning:** `-A` is very **noisy** — it generates significant traffic and is easily detected by IDS/IPS systems. Not stealthy.

---

#### 🔹 Fast Comprehensive Scan — Automated Workflow
```bash
...
> 📌 Demonstrates a typical **automated scanning pipeline** used in penetration testing engagements.

---

### 📊 Host Discovery Methods Compared
...
| ARP Ping | `-PR` | ARP | Scanning local network — most reliable on LAN |

---

## 2. Hping3 — Packet Crafting Specialist
...
> 💡 **Nmap vs Hping3 in one sentence:** Nmap *automates* scanning and *interprets* results; Hping3 gives you a *raw socket* and lets you **craft anything you want**.

---

### ⚖️ When to Use Hping3 vs Nmap
...
| No response | Packet is **filtered** — stateful firewall likely present |

> 📌 This mirrors Nmap's ACK scan logic: RST = unfiltered; silence = filtered.
...
## 3. Metasploit — Framework with Scanning Modules
...
> 💡 **Key Advantage — Modular Architecture:**
...
### 🔧 How Metasploit Is Used in Scanning
...
## 4. NetScanTools Pro — Multi-Function Suite
...
> 💡 Think of it as a GUI-based toolkit combining the functionality of dozens of individual command-line tools — useful for administrators and investigators who need comprehensive network intelligence without scripting.
...
| Category | Examples |
|----------|---------|
| **Active Tools** | Ping, port scanner, traceroute — directly probe targets |
| **Passive Tools** | WHOIS, DNS lookups — gather data without touching the target |
| **DNS Tools** | Zone transfer, MX/NS lookup, reverse DNS |
| **Local Computer Tools** | Network interface info, ARP table, routing table |

> 👉 This multi-category structure makes NetScanTools Pro a **complete reconnaissance suite** — covering both active scanning and passive intelligence gathering in one application.
...
|------|:----:|-------------|--------|
| **sx** | CLI Scanner | Extremely fast, stateless ARP/TCP/UDP scanner | [https://github.com/v-byte-cpu/sx](https://github.com/v-byte-cpu/sx) |
| **RustScan** | CLI Scanner | Ultra-fast port scanner (written in Rust); feeds results into Nmap | [https://github.com/RustScan/RustScan](https://github.com/RustScan/RustScan) |
| **MegaPing** | GUI Scanner | Windows GUI; ping sweep, port scan, NetBIOS scanner all in one | [http://magnetosoft.com](http://magnetosoft.com) |
| **SolarWinds Engineer's Toolset** | GUI Suite | Enterprise network monitoring + scanning toolkit | [https://www.solarwinds.com](https://www.solarwinds.com) |
| **PRTG Network Monitor** | GUI Suite | Continuous network monitoring with alerting and topology maps | [https://www.paessler.com](https://www.paessler.com) |

> 📌 **Exam Focus:** For CEH, focus on **Nmap**, **Hping3**, **Metasploit**, and **NetScanTools Pro** as the primary tools. The additional tools above are lower-priority but may appear in tool-identification questions.
...
## 7. CEH Exam Tips — Scanning Tools

| ⚠️ Exam Focus Area | ✅ What You Need to Know |
|--------------------|------------------------|
...
| **Why SYN scan is stealthy** | It never completes the three-way handshake → no full session logged |
...
| **Metasploit's key advantage** | **Modular architecture** — any exploit + any payload combination |
...
| **Silence in stealth scans** | Open port → *no response*; Closed port → *RST* (FIN/Xmas/NULL scans) |
