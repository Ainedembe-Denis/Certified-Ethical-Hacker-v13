...
1. [Overview of Network Scanning](#1-overview-of-network-scanning)
   ...
   - [Types of Scanning — Comparison Table](#-types-of-scanning--at-a-glance-comparison)
2. [Objectives of Network Scanning](#2-objectives-of-network-scanning--exam-critical)
   - [Objective 1 – Discover Live Hosts & Open Ports](#-objective-1--discover-live-hosts-ip-addresses-and-open-ports)
   - [Objective 2 – OS Fingerprinting](#-objective-2--os-and-architecture-fingerprinting)
   - [Objective 3 – Running Services](#-objective-3--discover-runninglistening-services)
   - [Objective 4 – Service Versions](#-objective-4--identify-specific-applications-and-service-versions)
   - [Objective 5 – Identify Vulnerabilities](#-objective-5--identify-vulnerabilities-in-network-systems)
   - [Objective 6 – Map Network Topology](#-objective-6--map-network-topology)
3. [TCP Communication Flags](#3-tcp-communication-flags)
   ...
   - [The Six TCP Flags](#-the-six-tcp-flags--in-depth)
     ...
     - [FIN](#-fin--finish)
     - [RST](#-rst--reset--)
4. [TCP/IP Communication](#4-tcpip-communication)
   - [The Three-Way Handshake](#-the-three-way-handshake)
   - [Connection Termination](#-connection-termination)
   - [Why Hackers Target the Handshake](#-why-hackers-target-the-handshake)
5. [Quick Reference Summaries](#5-quick-reference-summaries)
   - [TCP Flags at a Glance](#-tcp-flags-at-a-glance)
   - [Three Scanning Types Compared](#-three-scanning-types-compared)
   - [Examiner's Mental Model – Six Key Questions](#-examiners-mental-model--six-key-questions)
6. [CEH Exam Traps & Tips](#6-ceh-exam-traps--tips)

---

## 1. Overview of Network Scanning

Network scanning is a **systematic, active process** used by attackers and defenders alike to gather detailed intelligence about a target network. It is one of the most critical phases of the ethical hacking methodology — it transforms abstract information gathered during passive reconnaissance into **actionable attack intelligence**.

> 💡 **Analogy:** If *footprinting* is reading a city map from the library, then *scanning* is **walking the streets yourself** — knocking on doors, rattling windows, and checking which entrances are unlocked.

---

### 📍 Where Scanning Fits in the Hacking Lifecycle
...
[ Phase 2 ] Scanning  ◄── YOU ARE HERE
       ↓  Active — direct probing of systems
[ Phase 3 ] Enumeration
       ↓  Deeper data extraction (users, shares, policies)
[ Phase 4 ] Vulnerability Analysis
       ↓  Confirm exploitable weaknesses
[ Phase 5 ] Exploitation
       ↓  Gain access
[ Phase 6 ] Post-Exploitation / Maintaining Access
```

> 📌 **Exam Insight:** Scanning is **active reconnaissance**. Unlike passive footprinting, it generates traffic that could be logged or detected by IDS/IPS systems.
...
**Analogy:** Every port is like a *door or window* on a building. A port scanner walks the perimeter, testing every entrance — open, locked, or blocked behind a fence.
...
| **Filtered** | A firewall or ACL is blocking the probe — no response received |
...
> ⚠️ **Critical CEH Insight:** More open ports generally increase the attack surface — but even a system with **very few ports open** can be critically vulnerable if those ports run outdated or misconfigured services.

> 📌 **Exam Trap:** Do NOT assume fewer open ports always means a safer system.
...
> 💡 **CEH Context:** **Host discovery is a sub-step of network scanning.** The typical workflow is: *find live hosts → then port scan those specific hosts*. Running a port scan against an entire subnet is noisy and inefficient.
...
| Component | Role |
|-----------|------|
| **Scanning Engine** | Actively probes systems — sends test packets, analyzes responses, attempts safe exploits |
...
### 📊 Types of Scanning — At-a-Glance Comparison
...
| **Generates Traffic?** | ✅ Yes | ✅ Yes | ✅ Yes (significant) |
| **Can Trigger IDS?** | ✅ Yes | ✅ Limited | ✅ Yes (very likely) |

---

## 2. Objectives of Network Scanning *(Exam Critical)*

The CEH module defines **six clear objectives** that attackers pursue during scanning. These follow a natural attack progression — from discovering hosts all the way to planning exploitation.

> 🧩 **Think of it as building a puzzle:** Each objective adds a new piece of information until the attacker has a complete picture of the target environment.

---

### 🎯 Objective 1 – Discover Live Hosts, IP Addresses, and Open Ports
...
**Why it matters:**
> Open ports are **potential entry points**. This initial sweep tells the attacker *where* to focus precious attack time — toward the most accessible systems and services.
...
> ⚠️ **Exam Tip:** **ICMP (ping) is frequently blocked** by firewalls and host-based filters. Never assume a "no response to ping" means a host is offline — alternative techniques are required.

---

### 🎯 Objective 2 – OS and Architecture Fingerprinting
...
|--------|-------------|:-----------:|
| **Active Fingerprinting** | Sends specially crafted packets; analyzes TTL values, TCP window sizes, IP ID behavior, and protocol responses | ✅ Yes |
| **Passive Fingerprinting** | Silently observes captured network traffic without generating any probes | ❌ Much harder to detect |
...
> 📌 **Key Tools:** `nmap -O` (OS detection), `Xprobe2`, `p0f` (passive fingerprinting)

---

### 🎯 Objective 3 – Discover Running/Listening Services
...
> Open ports alone tell you *where a door is*. Service enumeration tells you *what kind of lock is on it* — and whether it's a cheap deadbolt or a vault door.
...
### 🎯 Objective 4 – Identify Specific Applications and Service Versions

This goes one level deeper than just service type — it targets **exact version numbers** of running applications and services.
...
|--------------|--------------|
| Apache **2.4.49** | CVE-2021-41773 — Path Traversal leading to RCE |
| OpenSSL **1.0.1 – 1.0.1f** | CVE-2014-0160 — Heartbleed |
| Windows SMBv1 | MS17-010 — EternalBlue (used in WannaCry) |
| Log4j **2.0–2.14.1** | CVE-2021-44228 — Log4Shell (JNDI injection RCE) |
| PHP **8.1.0-dev** | Backdoor in dev build — direct RCE |
| vsFTPd **2.3.4** | Backdoor introduced in compromised source package |

> 📌 **Tool:** `nmap -sV` (service and version detection), Netcat banner grabbing, Metasploit auxiliary scanners

> 💡 **Insight:** Version detection + a CVE database = a targeted, precision attack. No guessing required.

---

### 🎯 Objective 5 – Identify Vulnerabilities in Network Systems
...
```
Discover Host  →  Fingerprint OS  →  Enumerate Services  →  Detect Versions
       ↓
...
> 🔥 **This is where scanning transitions into exploitation planning.** Vulnerability data directly feeds into the attacker's toolset selection and attack path choice.

---

### 🎯 Objective 6 – Map Network Topology

Attackers don't stop at one system — they want a **complete picture of the entire environment**:

- Where are the routers, switches, and firewalls?
- Which network segments exist (DMZ, internal LAN, admin)?
- How do systems interconnect — which can communicate with which?
...
> 💡 **Tools:** `traceroute` / `tracert`, `nmap --traceroute`, Zenmap (visual graph), NetworkMiner, PathPing

---

## 3. TCP Communication Flags

TCP flags are **1-bit control fields** within the TCP header's 6-bit control field. They manage the *state* and *flow* of every TCP connection — from the initial handshake to graceful termination and abnormal resets.

> 💡 **Big Picture for CEH:** The **entire TCP scanning toolkit** (SYN scan, FIN scan, Xmas scan, NULL scan, ACK scan, etc.) is built entirely on **how different targets respond to specific, crafted TCP flag combinations**. Master the flags → master the scan types.

---

### 🔧 Why TCP Flags Matter in Scanning
...
> 👉 **Remember:** Many CEH scan type questions are answered entirely by asking *"What flags were sent?"* and *"What response came back?"*

---

### 🚩 The Six TCP Flags — In Depth
...
**Normal use — The Three-Way Handshake:**

```
Client  ──── SYN ──────────►  Server     (Step 1: Request connection)
Client  ◄─── SYN-ACK ───────  Server     (Step 2: Server acknowledges + requests)
Client  ──── ACK ──────────►  Server     (Step 3: Connection established)
```

**In Scanning — The SYN (Half-Open) Scan:**

```
Attacker ──── SYN ──────────►  Target
Attacker ◄─── SYN-ACK ───────  Target    (Port is OPEN)
Attacker ──── RST ──────────►  Target    (Abort — never complete the handshake)
```

> The attacker never completes the handshake → **no full connection is logged** → *stealth*.
...
> 📌 **If you see SYN in a CEH question → think:** connection initiation / half-open scan / stealth scan.
...
**In Scanning — ACK Scan (Firewall Detection):**

> If you send an **unsolicited ACK** (an ACK with no prior connection established), a live RFC-compliant host will reply with **RST** — regardless of whether the port is open or closed. However, if a **stateful firewall** is in the way, it will silently **drop** the unexpected ACK packet because it has no matching connection in its state table.
...
> 📌 **Key Rule:** ACK scan = **firewall detection tool**, NOT a port state detection tool.
...
> ⚠️ PSH is not individually significant in CEH scanning scenarios. Remember it as part of **Xmas = PSH + URG + FIN**.
...
> 📌 **Exam Appearance:** URG = Xmas scan ingredient. Alone, it is rarely tested.
...
Client  ◄─── ACK ───  Server      (OK, received)
Client  ◄─── FIN ───  Server      (I'm done too)
Client  ──── ACK ──►  Server      (Connection fully closed)
```

**In Scanning — The FIN Scan:**
...
|-----------------------|:---------------------:|----------|
| **Closed port** | `RST` — the host responds | Port is **Closed** |
| **Open port** | *Silence* — the packet is ignored | Port is **Open** |

> 💡 **Why FIN Scans Work Against Stateless Filters:**
> Stateless packet filters often only block incoming **SYN** packets from outside the network (to prevent unsolicited connection initiations). A FIN packet *looks like* it belongs to an already-established session, so it slips through — and the attacker reads the response.
...
> 💡 **The RST vs. No-Response distinction is the foundation of all stealth scanning logic.**
>
> - **Stealth scans** (FIN, Xmas, NULL) infer *open* ports from the **absence of RST**
> - They infer *closed* ports when **RST is received**

> 📌 **Single most important rule to memorize:**
...
TCP (**Transmission Control Protocol**) is a **connection-oriented** protocol — before any data is exchanged, both parties must first establish a reliable session through a structured handshake process. This fundamental behavior is exactly what attackers exploit during scanning.

---

### 🤝 The Three-Way Handshake

The three-way handshake is the mechanism TCP uses to **establish a connection** between a client and server. It synchronizes sequence numbers and confirms both sides are ready to communicate.
...
Client                              Server
  ...
  │             seq=x                 │           and sends initial sequence number
  │                                   │
  │◄─────────── SYN-ACK ─────────────│   Step 2: Server acknowledges + sends
  │             seq=y, ack=x+1        │           its own sequence number
  │                                   │
  │──────────── ACK ─────────────────►│   Step 3: Client confirms — connection
  │             ack=y+1               │           is now OPEN, data can flow
  │                                   │
  │◄══════════ DATA TRANSFER ════════►│
```
...
| Step | Packet Sent | Direction | Purpose |
|:----:|:-----------:|:---------:|--------|
| **1** | `SYN` | Client → Server | Request a new connection; share client's initial sequence number |
| **2** | `SYN-ACK` | Server → Client | Acknowledge client's SYN; share server's sequence number |
| **3** | `ACK` | Client → Server | Confirm server's sequence number; **connection is established** |

> ✅ After Step 3, both sides are synchronized and **data transfer begins**.

---

### � Connection Termination

TCP connections are closed in one of two ways:

| Method | Flag Used | Behavior |
|--------|:---------:|----------|
| **Graceful close** | `FIN` | Orderly four-step shutdown — both sides finish cleanly |
| **Forced termination** | `RST` | Immediate abort — connection is torn down without ceremony |

**Normal Four-Way FIN Teardown:**
...
> 📌 **Note:** After sending its FIN, a host enters `TIME_WAIT` state before fully closing, ensuring any delayed packets are absorbed and not misinterpreted by a new connection.

---

### 🔥 Why Hackers Target the Handshake

The three-way handshake is the core of **TCP stealth scanning**. By deliberately **interrupting or manipulating** the handshake, attackers gain significant tactical advantages:

| Attacker Action | What Happens | Benefit |
|-----------------|-------------|--------|
| Send `SYN`, receive `SYN-ACK`, **then send `RST`** | Handshake never completes | Connection is **never logged** by the target |
| Send only `FIN` or `URG+PSH+FIN` (no SYN at all) | Target responds differently based on port state | Port state revealed **without initiating a session** |
| Never complete the handshake at all | Half-open connection attempt | Bypasses some **application-layer logging** |

**Hacker's Tactical Goals:**

- 🕵️ **Stay stealthy** — avoid creating a full, logged TCP session
- 📋 **Avoid connection logs** — half-open scans don't appear in most app logs
- ⚡ **Quickly test thousands of ports** — faster than completing full handshakes
- 🔓 **Bypass stateless filters** — non-SYN packets slip through rules blocking only SYN

> 📌 **Exam Trap:** Know that **SYN scanning (`-sS` in Nmap)** intentionally stops **before completing the three-way handshake**. This is what makes it a *stealth* scan — and why it requires raw socket privileges (root/admin).

## 5. Quick Reference Summaries
...
### 🧠 Examiner's Mental Model — Six Key Questions
...
> ✅ **Exam Strategy:** In every CEH scanning question, ask which of these 6 questions is being answered. The answer reveals the correct technique and often the correct tool.

---

## 6. CEH Exam Traps & Tips
...
| **Assuming ICMP ping always works** | ICMP is frequently **blocked by firewalls** — TCP/UDP/ARP-based host discovery may be required |
| **Underestimating OS fingerprinting** | It is a **defined scanning objective**, not just optional extra recon |
| **Ignoring topology mapping** | It is critical for lateral movement planning — an attacker needs the full network picture |
| **Assuming fewer open ports = safer** | A single port running a critically vulnerable service is all an attacker needs |
| **Thinking FIN scan works everywhere** | FIN scans **do not work reliably on Windows** — Windows responds with RST on both open and closed ports |
...
> 💡 **Bonus — Common CEH Tools Summary:**
>
> | Tool | Primary Use |
> |------|------------|
