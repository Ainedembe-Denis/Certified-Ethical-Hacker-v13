...
> 💡 **Analogy:** If modern protocols are cell phones, NetBIOS is an **old intercom system** in an office building. It's often still plugged in, broadcasting names and room numbers to anyone listening.

### 🏷️ NetBIOS Names & Suffixes
...
### 🔍 Interpreting Output (Common Suffixes)
...
| Tool | Capability |
| :--- | :--- |
| **Nmap (nbstat.nse)** | `nmap -sV -v --script nbstat.nse <target>` — Retrieves name table and MAC address. |
...
| Tool | Primary Purpose | Attack Vector |
| :--- | :--- | :--- |
| **PsExec** | Remote Execution | Launch interactive command prompts on remote systems. |
| **PsLoggedOn** | Find Logged-In Users | Identify machines where a **Domain Admin** is currently active. |
| **PsGetSid** | SID Translation | Map SIDs to user names and vice versa (Admin = RID 500). |
| **PsInfo** | System Recon | Gather OS version, installation date, RAM, and CPU type. |
| **PsFile** | List Remote Files | View (and close) files opened remotely on a system. |
| **PsList** | Resource Stats | View CPU, memory, and thread statistics for remote processes. |
| **PsKill** | Terminate Processes | Remotely kill antivirus, EDR, or security monitoring tools. |
| **PsLogList** | Dump Event Logs | Remote elogdump; reveals logins, errors, and security events. |
| **PsPasswd** | Change Passwords | Perform mass password changes across managed systems via APIs. |
| **PsShutdown** | Remote Control | Trigger a shutdown or reboot to perform a Denial-of-Service (DoS). |
...
- **Prompt:** *"Perform NetBIOS enumeration on 10.10.1.11"* → **Result:** `nbtscan 10.10.1.11`
- **Prompt:** *"Get NetBIOS info with nmblookup"* → **Result:** `nmblookup -A 10.10.1.11`
- **Prompt:** *"Enumerate NetBIOS on 10.1.1.22 using nmap"* → **Result:** `nmap -sU -p 137 --script nbstat.nse 10.1.1.22`
...
| **PsExec vs Telnet** | PsExec is a "lightweight Telnet replacement"—no client install needed. |
...
Part of the **Sysinternals** suite, these are legitimate admin tools that attackers use for remote control without needing any client-side installation.

| Tool | Primary Purpose | Attacker Use Case |
| :--- | :--- | :--- |
| **PsExec** | Remote Execution | Execute commands/shells on remote systems (Hacker-friendly Telnet). |
| **PsLoggedOn** | Find Logged-In Users | Identify machines where a **Domain Admin** is logged in. |
| **PsGetSid** | SID Translation | Map out Security Identifiers to identify sensitive accounts (Admin = RID 500). |
| **PsInfo** | System Recon | Gather OS version, RAM, and uptime to tailor exploits. |
| **PsFile** | List Remote Files | See which sensitive files are currently being accessed by users. |
| **PsKill** | Terminate Processes | Remotely kill antivirus or security monitoring software. |

---

## 5. Enumerating Shared Resources (Net View)

The `net view` command allows you to browse the "Network Neighborhood" from the CLI.

- `net view \\<IP>` : Lists visible file and printer shares.
- `net view \\<IP> /ALL` : **The Attacker's Choice.** Shows all shares, including **hidden administrative shares** (ends with `$`, like `C$`, `ADMIN$`).
- `net view /domain` : Lists all domains on the network.
- `net view /domain:<name>` : Lists all computers within a specific domain.

---

## 6. AI-Powered NetBIOS Enumeration

Modern attackers use AI (ChatGPT/LLMs) to generate complex enumeration syntax instantly.

- **Prompt:** *"Perform NetBIOS enumeration on 10.10.1.11"* → **Result:** `nbtscan 10.10.1.11`
- **Prompt:** *"Get NetBIOS info and name with nmap"* → **Result:** `nmap -sU -p 137 --script nbstat.nse 10.10.1.11`
- **Prompt:** *"Find logged in users on 10.1.1.22"* → **Result:** `psloggedon \\10.1.1.22`
...
| ⚠️ Common Trap | ✅ Correct Understanding |
| :--- | :--- |
| **Confusion on Ports** | **137 (UDP)** = Names; **139 (TCP)** = Sessions/Sharing. |
| **The <20> Suffix** | Always indicates the **Server Service** (File Sharing) is active. |
| **PsExec vs Shell** | PsExec provides a lightweight, authentic remote shell without a full Telnet/SSH service. |
| **Hidden Shares ($)** | The `/ALL` switch is required to see administrative shares like `ADMIN$` or `C$`. |
| **NetBIOS vs SMB** | NetBIOS is the legacy transport; **SMB over Port 445** is the modern direct-host method. |
